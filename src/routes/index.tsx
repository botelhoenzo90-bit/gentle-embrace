...
import {
  ArrowRight, Award, Building2, Check, ChevronDown, ChevronLeft, ChevronRight,
  ClipboardCheck, Droplets, Hammer, Home, MapPin, Menu, MessageCircle, ShieldCheck,
  Sparkles, Star, Target, Wrench, X, Zap
} from "lucide-react";
...
function Index() {
  ...
  useEffect(() => {
    const timer = window.setInterval(() => {
      setServiceIndex((current) => (current + 1) % Math.max(1, services.length - 2));
    }, 4500);
  ...
  return (
    <div className="ae-site">
      <div className="ae-topbar"><div className="ae-container ae-topbar-inner"><span><MapPin size={14}/> Brasília • DF</span><span>Engenharia Civil • Reformas • Telhados • Laudos</span><a href={wa("Olá! Quero falar com a Amaral Engenharia.")} target="_blank">(61) 98327-7032</a></div></div>
      <header className="ae-header">
        <div className="ae-container ae-nav">
          <a className="ae-brand" href="#inicio"><span className="ae-logo"><span>A</span><span>E</span></span><span><b>Amaral Engenharia</b><small>Engenharia Civil • Perícias</small></span></a>
          <nav className={menuOpen ? "ae-menu ae-menu-open" : "ae-menu"}>
            <a href="#servicos" onClick={() => setMenuOpen(false)}>Serviços</a><a href="#telhados" onClick={() => setMenuOpen(false)}>Telhados</a><a href="#empresa" onClick={() => setMenuOpen(false)}>A empresa</a><a href="#regioes" onClick={() => setMenuOpen(false)}>Regiões</a><a href="#faq" onClick={() => setMenuOpen(false)}>Dúvidas</a>
          </nav>
          <a className="ae-btn ae-btn-red ae-nav-btn" href={wa("Olá! Quero solicitar um orçamento para meu imóvel.")} target="_blank"><MessageCircle size={17}/> Solicitar orçamento</a>
          <button className="ae-mobile-toggle" onClick={() => setMenuOpen(!menuOpen)} aria-label="Abrir menu">{menuOpen ? <X/> : <Menu/>}</button>
        </div>
      </header>
      ...
      <main>
        <section className="ae-hero" id="inicio">
          ...
          <div className="ae-container ae-hero-grid">
            <div className="ae-hero-copy">
              ...
              <div className="ae-actions"><a className="ae-btn ae-btn-red ae-btn-big" href={wa("Olá! Quero solicitar uma avaliação para meu imóvel.")} target="_blank"><MessageCircle size={19}/> Falar com o engenheiro</a><a className="ae-btn ae-btn-outline" href="#servicos">Conhecer serviços <ArrowRight size={17}/></a></div>
            ...
            <div className="ae-hero-card">
              <div className="ae-blueprint"><div className="blueprint-lines"/><span>AMARAL<br/><b>ENGENHARIA</b></span><small>PROJETO • REFORMA • INSPEÇÃO</small></div>
              <div className="ae-floating-card"><div className="ae-mini-icon"><ShieldCheck/></div><div><b>Atendimento técnico</b><span>Regiões selecionadas do DF</span></div></div>
            </div>
          </div>
        </section>
        ...
        <section className="ae-section ae-services" id="servicos">
          <div className="ae-container"><div className="ae-heading"><div><span className="ae-kicker">SOLUÇÕES COMPLETAS</span><h2>Um só contato para cuidar do seu imóvel.</h2></div><p>Os panfletos da empresa apresentam uma atuação ampla, com destaque para reformas, telhados e serviços técnicos de engenharia.</p></div>
            <div className="ae-service-window"><div className="ae-service-track" style={{transform:`translateX(-${serviceIndex * 33.333}%)`}}>{services.map((service, i) => {const Icon=service.icon; return <article className="ae-service-card" key={service.title}><div className="ae-card-top"><div className="ae-icon"><Icon/></div><span>0{i+1}</span></div><h3>{service.title}</h3><p>{service.text}</p><a href={wa(`Olá! Tenho interesse em: ${service.title}. Quero mais informações.`)} target="_blank">Solicitar atendimento <ArrowRight size={15}/></a></article>})}</div></div>
            <div className="ae-slider-controls"><button onClick={() => setServiceIndex(Math.max(0, serviceIndex - 1))}><ChevronLeft/></button><div className="ae-dots">{[0,1,2,3].map(n=><i key={n} className={serviceIndex===n?"active":""}/>)}</div><button onClick={() => setServiceIndex(Math.min(3, serviceIndex + 1))}><ChevronRight/></button></div>
            <div className="ae-all-services"><strong>Também realizamos:</strong> administração e fiscalização de obras • projetos • substituição de telhas • cumeeiras • ripamento • madeiramento • impermeabilização • pisos • revestimentos • pintura • grafiato • gesso • Drywall • piscina • sauna • varanda • churrasqueira • forno e fogão a lenha.</div>
          </div>
        </section>

        <section className="ae-section ae-about" id="empresa"><div className="ae-container ae-about-grid"><div className="ae-visual"><div className="ae-photo-main"/><div className="ae-photo-label"><span>AMARAL</span><b>ENGENHARIA</b><small>Construção • Reforma • Laudo • Perícia • Avaliação • ART</small></div><div className="ae-stamp"><Award/><b>2 ANOS</b><span>DE GARANTIA*</span></div></div><div className="ae-copy"><span className="ae-kicker">QUEM SOMOS</span><h2>Experiência técnica para decisões mais seguras.</h2><p>A Amaral Engenharia, representada por Valluce Amaral, Engº Civil • CREA 27125 / D MG, oferece soluções para manutenção, reforma e necessidades técnicas de imóveis em Brasília.</p><ul>{["Satisfação, rapidez e preço justo como pilares divulgados pela empresa.","Atuação regional focada em Lago Norte, Lago Sul, Jardim Botânico e Plano Piloto.","Serviços de engenharia, reformas, telhados, avaliações, perícias e laudos.","Atendimento direto para entender o problema antes de indicar o serviço."] .map(t=><li key={t}><Check size={17}/>{t}</li>)}</ul><a className="ae-btn ae-btn-red" href={wa("Olá! Quero conhecer os serviços da Amaral Engenharia.")} target="_blank">Conversar com a Amaral Engenharia <ArrowRight size={17}/></a></div></div></section>

        <section className="ae-section ae-roof" id="telhados"><div className="ae-container"><div className="ae-heading ae-heading-light"><div><span className="ae-kicker">PRINCIPAL ESPECIALIDADE</span><h2>Revisão de telhado sem deixar o problema para depois.</h2></div><p>O material enviado destaca revisão, reformas, substituição de telhas e diversos serviços de proteção e impermeabilização.</p></div><div className="ae-roof-layout"><div className="ae-roof-visual"><div className="ae-roof-sky"/><div className="ae-roof-house"/><div className="ae-roof-badge"><Wrench/><b>REVISÃO<br/>DE TELHADO</b><span>AVALIAÇÃO • REPARO • PREVENÇÃO</span></div></div><div className="ae-roof-list">{roofItems.map((item,i)=><div key={item}><span>{String(i+1).padStart(2,"0")}</span><Check size={17}/><b>{item}</b></div>)}<a className="ae-btn ae-btn-red" href={wa("Olá! Quero agendar uma avaliação/revisão do meu telhado.")} target="_blank">Quero avaliar meu telhado <ArrowRight size={17}/></a></div></div></div></section>

        <section className="ae-section"><div className="ae-container"><div className="ae-heading"><div><span className="ae-kicker">SERVIÇOS TÉCNICOS</span><h2>Quando a decisão exige conhecimento de engenharia.</h2></div><p>Além da execução, os panfletos apresentam serviços técnicos para documentação, avaliação e análise de imóveis e obras.</p></div><div className="ae-tech-grid"><div className="ae-tech-card"><ClipboardCheck/><h3>Laudo Técnico</h3><p>Documentação técnica para registrar e analisar condições do imóvel.</p><a href={wa("Olá! Preciso de informações sobre Laudo Técnico.")} target="_blank">Solicitar informações <ArrowRight/></a></div><div className="ae-tech-card"><Target/><h3>Perícia & Avaliação</h3><p>Perícia e avaliação de apartamento, casa e situações relacionadas à obra.</p><a href={wa("Olá! Preciso de informações sobre Perícia ou Avaliação.")} target="_blank">Falar com o engenheiro <ArrowRight/></a></div><div className="ae-tech-card"><Award/><h3>ART & Projetos</h3><p>ART, projetos, administração e fiscalização de obras conforme a necessidade.</p><a href={wa("Olá! Preciso de informações sobre ART ou Projeto.")} target="_blank">Quero atendimento <ArrowRight/></a></div></div></div></section>

        <section className="ae-section ae-process"><div className="ae-container"><div className="ae-heading"><div><span className="ae-kicker">COMO FUNCIONA</span><h2>Do primeiro WhatsApp à solução.</h2></div></div><div className="ae-process-grid">{[["01","Converse conosco","Explique o problema, serviço ou reforma que você precisa."],["02","Envie detalhes","Fotos, localização e informações ajudam a entender a demanda."],["03","Avaliação técnica","A necessidade é direcionada para o serviço adequado."],["04","Execução","Planejamento e realização do serviço com acompanhamento."]].map(([n,t,d])=><div key={n}><span>{n}</span><div><h3>{t}</h3><p>{d}</p></div></div>)}</div></div></section>

        <section className="ae-section ae-areas" id="regioes"><div className="ae-container"><div className="ae-heading"><div><span className="ae-kicker">REGIÕES DE ATENDIMENTO</span><h2>Atendimento onde a empresa escolheu atuar.</h2></div><p>O cliente informou atendimento exclusivo nestas regiões de Brasília/DF.</p></div><div className="ae-area-grid">{areas.map((area,i)=><div key={area}><div className="ae-area-number">0{i+1}</div><MapPin/><h3>{area}</h3><span>Brasília • DF</span><a href={wa(`Olá! Estou no ${area} e quero verificar atendimento.`)} target="_blank">Verificar atendimento <ArrowRight/></a></div>)}</div></div></section>

        <section className="ae-cta"><div className="ae-container ae-cta-inner"><div><span className="ae-kicker">FALE DIRETAMENTE COM A ENGENHARIA</span><h2>Tem vazamento, telhado com problema ou uma reforma para fazer?</h2><p>Envie uma mensagem. Se puder, mande fotos e explique o que está acontecendo para agilizar o atendimento.</p></div><a className="ae-btn ae-btn-white ae-btn-big" href={wa("Olá! Vim pelo site da Amaral Engenharia e quero solicitar um orçamento.")} target="_blank"><MessageCircle/> Solicitar orçamento pelo WhatsApp <ArrowRight/></a></div></section>
